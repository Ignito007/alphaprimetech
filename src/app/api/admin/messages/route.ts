...
export async function GET() {
  try {
    ...
    // We try multiple common filenames so this works even if your current contact route uses a different one.
    const candidates = [
      path.join(submissionsDir, "contact.jsonl"),
      path.join(submissionsDir, "contacts.jsonl"),
      path.join(submissionsDir, "messages.jsonl"),
      path.join(submissionsDir, "contact-messages.jsonl"),
    ];
    ...
    return NextResponse.json({ ok: false, error: "Failed to read messages." }, { status: 500 });
  }
}
